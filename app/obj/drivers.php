...
class objDrivers{
    ...
    public function getDriversList(){
        ...
        $db = new libDatabase;
		$drivers = $db->selectAllDriversAsArray();
		$listHTML ='';
		foreach ($drivers as $driver) {
		  //  $listHTML .= '<p class="bg-info"><span class="listDrvNo">'.$driver['driverID'].' ('.$driver['driverName'].')</span>';
		  //  $listHTML .= '<a href="/settings/driver/delete/?driverID='.$driver['driverID'].'" class="btn btn-default"><i class="fa fa-trash"></i> DELETE</a>';
		  //  $listHTML .= '</p>';
		    
            $listHTML .= '<div class="card card-stats card-status-driver">
                    <div class="card-header card-header-info card-header-icon">
                        <div class="card-icon">
                            <i class="material-icons">'.$driver['driverID'].'</i>
                        </div>
                        <p class="card-category">'.$driver['driverName'].'.
                        <a href="/drivers/delete/?driverID='.$driver['driverID'].'" class="btn btn-default btndelete"><i class="fa fa-trash"></i></a>
                        </p>
                    </div>
                </div>';
    ...
    public function getDriversFromCSV(){
        $file = taxiRent_APP_DIR."drv.csv";
    	$csv = array_map('str_getcsv', file($file));
    	echo "<pre>";
    	for ($i = 0; $i < count($csv); $i++) {
    		echo $query = 'INSERT INTO drivers (driverID, driverName) VALUES ("'.$csv[$i][0].'", "'.$csv[$i][1].'");<br/>';
    	}
    	echo "</pre>";
    }
    
    public function SW_ImplodeCSV(array $rows, $headerrow=true, $mode='EXCEL', $fmt='2D_FIELDNAME_ARRAY')
    ...
    public function SW_ExplodeCSV($csv, $headerrow=true, $mode='EXCEL', $fmt='2D_FIELDNAME_ARRAY')
